...
if (isset($_GET["id"])) {
    ...
    while (($row = oci_fetch_assoc($stid)) != false) {
        ...
        //session ma null value set garna mildaina , agadi ta variable ma store bhako cha if aba discount_id chadaichaina bhane session ma store huni vayena
        //incase discount id cha bhane matra session ma discount id bhanera store garni

        if (!is_null($discount_id)) {
            $querys = "SELECT * FROM discount WHERE DISCOUNT_ID = $discount_id";
            $stdis = oci_parse($connection, $querys);
            oci_execute($stdis);
            while (($row = oci_fetch_assoc($stdis)) != false) {
                $discount_id = $row["DISCOUNT_ID"];
                $discount_percent = $row["DISCOUNT_PERCENT"];
                $_SESSION["p_discount_id"] = $discount_id;
                $_SESSION["p_discount_percent"] = $discount_percent;
            }
        }
    }
}
...
<body>
    <form action="update_product.php" method="post" enctype='multipart/form-data'>
        ...
        <div class=" row mt-2">
            ...
            <div class="col-md-6"><label class="labels">Discount % </label><input type="number" class="form-control" name="discount" min="1" max="100" value="<?php echo $_SESSION["p_discount_percent"]; ?>"></div>
            <div class="col-md-6">
                ...
                <!--selected attribute is added to that option, making it the default selected option when the page loads.-->
